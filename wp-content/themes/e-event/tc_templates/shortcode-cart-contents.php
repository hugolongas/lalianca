		...
		<form id="tickera_cart" method="post" class="tickera" name="tickera_cart">
					...
					<table cellspacing="0" class="tickera_table" cellpadding="10">
						...
						<tbody>
							...
							foreach ( $cart_contents as $ticket_type => $ordered_count ) {
								...
								?>
								<tr class="tickets-list">
									<td class="ticket-event-name"><span class="point"><i></i></span><span class="title"><?php echo get_the_title(get_post_meta($ticket->details->ID, 'event_name', true)); ?></span></td>
							...
							<?php 
								$tc_general_settings = get_option( 'tc_general_setting', false );
								$cols = isset($tc_general_settings[ 'use_global_fees' ]) ? '6' : '5';
							?>
							<tr class="last-table-row">
								<td class="ticket-total-all" colspan="<?php echo apply_filters( 'tc_cart_table_colspan', $cols ); ?>">
							...
							<tr>
								<td class="actions" colspan="<?php echo apply_filters( 'tc_cart_table_colspan', $cols ); ?>">
									<?php do_action( 'tc_cart_before_discount_field' ); ?>
									<?php
									 	$hidden_fields = !isset( $tc_general_settings[ 'show_discount_field' ] ) || (isset( $tc_general_settings[ 'show_discount_field' ] ) && $tc_general_settings[ 'show_discount_field' ] == 'yes') ? '' : 'hidden';
									 ?>
										<span class="coupon-code <?php print $hidden_fields;?>"><input type="text" name="coupon_code" id="coupon_code" placeholder="<?php _e( "Discount Code", "tc" ); ?>" class="coupon_code tickera-input-field" value="<?php echo esc_attr( (isset( $_POST[ 'coupon_code' ] ) ? $_POST[ 'coupon_code' ] : (isset( $_COOKIE[ 'tc_discount_code_' . COOKIEHASH ] ) ? $_COOKIE[ 'tc_discount_code_' . COOKIEHASH ] : '') ) ); ?>" /></span> <input type="submit" id="apply_coupon" value="<?php _e( "Apply Coupon", "tc" ); ?>" class="apply_coupon tickera-button <?php print $hidden_fields;?>"><span class="coupon-code-message <?php print $hidden_fields;?>"><?php echo apply_filters( 'tc_discount_code_message', '' ); ?></span><?php do_action( 'tc_cart_after_discount_field' ); ?>
			...
			<div class="tickera_additional_info">
				...
				<div class="tickera_buyer_info info_section">
					<h3 class="site-title"><?php _e('Buyer','tc');?> <span><?php _e('Info','tc');?></span><i></i></h3>
					<?php
					...
					foreach ( $buyer_form_fields as $field ) {
						...
						?><?php if ( $field[ 'field_type' ] == 'text' ) { ?><div class="fields-wrap <?php
							...
							?>"><label><input type="<?php echo $field[ 'field_type' ]; ?>" <?php
										 if ( isset( $field[ 'field_placeholder' ] ) ) {
											 echo 'placeholder="' . esc_attr( $field[ 'field_placeholder' ] ) . '"';
										 }
										 ?> class="buyer-field-<?php echo $field[ 'field_type' ] . ' ' . $validation_class; ?> tickera-input-field check-value" value="<?php echo (isset( $_POST[ 'buyer_data_' . $field[ 'field_name' ] . '_' . $field[ 'post_field_type' ] ] ) ? $_POST[ 'buyer_data_' . $field[ 'field_name' ] . '_' . $field[ 'post_field_type' ] ] : $buyer_form->get_default_value( $field )); ?>" name="<?php echo 'buyer_data_' . $field[ 'field_name' ] . '_' . $field[ 'post_field_type' ]; ?>"><span><?php echo ($field[ 'required' ] ? '*' : ''); ?><?php echo $field[ 'field_title' ]; ?></span></label><span class="description"><?php echo $field[ 'field_description' ]; ?></span></div><!-- fields-wrap --><?php } ?>
				...
				?>>
						 ...
						 foreach ( $cart_contents as $ticket_type => $ordered_count ) {
							 ...
							 $ticket = new TC_Ticket( $ticket_type );
							 ?>
						<h2 class="site-title"><?php echo __('Ticket ','tc').'<span>'.apply_filters( 'tc_checkout_owner_info_ticket_title', $ticket->details->post_title, $ticket_type, $cart_contents ).'</span>'; ?><i></i></h2>
						<?php
								...
								<?php foreach ( $owner_form_fields as $field ) { ?>
									...
									if ( $show_owner_fields ) {
										...
										<?php if ( $field[ 'field_type' ] == 'text' ) { ?>
											<?php if ( (isset( $tc_general_settings[ 'show_owner_email_field' ] ) && $tc_general_settings[ 'show_owner_email_field' ] == 'yes' && $field[ 'field_name' ] == 'owner_email' ) || $field[ 'field_name' ] !== 'owner_email' ) { ?><div class="fields-wrap <?php
												...
												?>"><label><input type="<?php echo $field[ 'field_type' ]; ?>" <?php
															 ...
															 ?> class="owner-field-<?php echo $field[ 'field_type' ] . ' ' . $validation_class; ?> tickera-input-field tc-owner-field <?php if($field[ 'field_name' ] == 'owner_email' ){ ?>tc_owner_email<?php } ?> check-value" value="" name="<?php echo 'owner_data_' . $field[ 'field_name' ] . '_' . $field[ 'post_field_type' ]; ?>[<?php echo $ticket_type; ?>][<?php echo $owner_index; ?>]"><span><?php echo ($field[ 'required' ] ? '*' : ''); ?><?php echo $field[ 'field_title' ]; ?></span></label><span class="description"><?php echo $field[ 'field_description' ]; ?></span></div><!-- fields-wrap --><?php } ?><?php } ?>
										...
										<?php if ( $field[ 'field_type' ] == 'textarea' ) { ?><div class="fields-wrap <?php
											...
											?>"><label><textarea class="owner-field-<?php echo $field[ 'field_type' ] . ' ' . $validation_class; ?> tickera-input-field check-value" <?php
												 if ( isset( $field[ 'field_placeholder' ] ) ) {
													 echo 'placeholder="' . esc_attr( $field[ 'field_placeholder' ] ) . '"';
												 }
												 ?> name="<?php echo 'owner_data_' . $field[ 'field_name' ] . '_' . $field[ 'post_field_type' ]; ?>[<?php echo $ticket_type; ?>][<?php echo $owner_index; ?>]"></textarea><span><?php echo ($field[ 'required' ] ? '*' : ''); ?><?php echo $field[ 'field_title' ]; ?></span></label><span class="description"><?php echo $field[ 'field_description' ]; ?></span></div><!-- fields-wrap --><?php } ?>
				...
				<p><input type="submit" id="proceed_to_checkout" name="proceed_to_checkout" value="<?php _e( "Checkout", "tc" ); ?>" class="tickera_checkout tickera-button"></p>
