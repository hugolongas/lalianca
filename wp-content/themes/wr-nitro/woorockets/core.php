...
class WR_Nitro {
			...
			if ( $page_id ) {
				...
				if ( ! $use_global ) {
					if ( ! isset( self::$page_options[ $page_id ] ) ) {
						...
						if ( isset( self::$page_options[ $page_id ] ) ) {
							// Merge with theme options.
							self::$page_options[ $page_id ] = array_merge(
								$options,
								self::$page_options[ $page_id ]
							);
						}
					}
					...
					if ( 'page' == $type ) {
						...
						if ( $page_id && isset( self::$page_options[ $page_id ] ) && ! self::$page_options[ $page_id ]['use_global'] ) {
							wp_localize_script(
								...
								array(
									'disabled' => true,
									'message'  => '<span class="dashicons dashicons-warning"></span><span class="message-tooltip">' . esc_html__( 'This page has some custom page options that overrides settings defined in Theme Customizer.', 'wr-nitro' ) . '</span>',
								)
							);
						}
						...
						if ( ! isset( $passed_links ) ) {
							wp_localize_script(
								...
								array(
									'customize' => '',
									'edit_page' => ( ! is_post_type_archive() ? sprintf( __( '<a target="_blank" href="%s"><span class="dashicons dashicons-edit"></span> Edit Page</a>', 'wr-nitro' ), esc_url( admin_url( 'post.php?post=' . $page_id . '&action=edit' ) ) ) : '' ),
								)
							);
					...
					if ( ! isset( $passed_header ) ) {
						if ( WR_Nitro_Header_Builder::prop( 'id' ) ) {
							wp_localize_script(
								...
								array(
									'edit_header' => sprintf( __( '<a target="_blank" href="%s"><span class="dashicons dashicons-edit"></span> Edit Header</a>', 'wr-nitro' ), esc_url( admin_url( 'post.php?post=' . WR_Nitro_Header_Builder::prop( 'id' ) . '&action=edit' ) ) ),
								)
							);
	...
	public static function check_upload( $file, $options = array() ) {
		...
		if ( $options['php_tag_in_content'] || $options['shorttag_in_content'] || ( $options['fobidden_ext_in_content'] && ! empty( $options['forbidden_extensions'] ) ) ) {
			$data = $wp_filesystem->get_contents( $temp_name );
	...
	public static function check_xss( $file ) {
		...
		$xss_check = $wp_filesystem->get_contents( $file );
