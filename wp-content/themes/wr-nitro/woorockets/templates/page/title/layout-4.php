...
?>
<div class="site-title style-4 pr <?php echo esc_attr( implode( ' ', $wr_classes ) ) . ' ' . ( is_customize_preview() ? 'customizable customize-section-page_title ' : '' ); ?>" <?php echo implode( '', $wr_attr ); ?>>
	<div class="container tc">
		<?php do_action( 'nitro_top_of_page_title' ); ?>
		<h1>
			<?php // Render page title
				WR_Nitro_Helper::page_title();
			?>
		</h1>
		<?php
		...
		<?php
			...
			if ( ! $wr_nitro_options['use_global'] && ! empty( $wr_page_desc ) ) {
				echo '<p class="desc"> ' . $wr_nitro_options['wr_page_title_desc'] . '</p>';
			}
		?>
