				...
				<?php
					echo '<div class="entry-thumb tc mgb30 ' . esc_attr( $wr_class ) . '" ' . $wr_data . '>';
						if ( $wr_format == 'gallery' ) {
							...
							echo '<div class="quote-content pr tc color-white pd30 pdt40 pdb40" ' . ( ( ! empty( $wr_image ) ) ? 'style="background: url(' . esc_url( $wr_image ) . ');background-size: cover"' : '' ) . '><div class="mask"></div><p class="color-white pr">' . esc_html( $wr_content ) . '</p><span class="pr"><strong>' . esc_html( $wr_author ) . '</strong></span></div>';
				...
				<div class="entry-content" >
					...
					<?php
						if ( $wr_nitro_options['blog_single_author'] ) {
							$wr_author = sprintf(
								__( '<div class="post-author overlay_bg">%1$s%2$s</div>', 'wr-nitro' ),
								'<div class="pull-left">' . get_avatar( get_the_author_meta( 'user_email' ), '68', '' ) . '</div>',
								'<div class="author-info"><a class="name" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a><p>' . get_the_author_meta( 'description' ) . '</p></div>'
