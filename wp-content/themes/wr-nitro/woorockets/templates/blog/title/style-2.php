...
?>
<div class="post-title style-2 fc mgb30 pr<?php echo esc_attr( $wr_class ) . ( is_customize_preview() ? ' customizable customize-section-blog_single' : '' ); ?>" style="<?php echo esc_attr( implode( ' ', $wr_style ) ); ?>" <?php if ( isset( $wr_data[1] ) ) echo wp_kses_post( $wr_data[1] ); ?>>
	<div class="container tc pr" <?php if ( isset( $wr_data[2] ) ) echo wp_kses_post( $wr_data[2] ); ?>>
		<h1 class="entry-title mg0 mgb30"><?php the_title(); ?></h1>
		<?php
			echo sprintf(
				__( '<div class="entry-author">%1$s<span class="db mgt10">%2$s</span></div>', 'wr-nitro' ),
