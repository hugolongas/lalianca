...
$wr_is_revslider_activated   = is_plugin_active( 'revslider/revslider.php' );
$wr_is_woocommerce_activated = is_plugin_active( 'woocommerce/woocommerce.php' );
$wr_is_wrls_activated        = is_plugin_active( 'wr-live-search/main.php' );
$wr_is_wrcc_activated        = ( $wr_is_woocommerce_activated && is_plugin_active( 'wr-currency/main.php' ) ) ? true : false;
...
$wr_nitro_options['wc_general_wishlist'] == 1 && (
	is_plugin_active( 'yith-woocommerce-wishlist/init.php' ) || 
	is_plugin_active( 'yith-woocommerce-wishlist-premium/init.php' )
) ) ? true : false;

$wr_nitro_header_html[] = '<div class="header-outer clear ' . $wr_class_position . '" data-id="' . WR_Nitro_Header_Builder::prop( 'id' ) . '">';
		...
		foreach ( $wr_nitro_header_data['rows'] as $key => $val ) {
							...
							switch ( $wr_hb_item['_rel'] ) {
								...
								case 'search':
									...
									$wr_nitro_header_html[] = '
										...
										$wr_nitro_header_html[] = '
														<form class="' . $wr_class_outer . '" action="' . esc_url( home_url( '/' ) ) . '" method="get" role="search">
									...
									foreach( $wr_hb_item['style'] as $key_sub_item => $val_sub_item ) {
										...
										if ( $key_sub_item == 'backgroundImage' ) {
											...
											$wr_item_css .= esc_attr( $wr_attr_css ) . ':' . esc_html( $val_sub_item ) . ';';
										}
									}
									...
									if ( ! $wr_is_mobile ) {
										...
										$wr_nitro_header_html[] = '
											...
											if( (int) $wr_hb_item['menuID'] > 0 && is_nav_menu( (int) $wr_hb_item['menuID'] ) ) {
												...
												$wr_nitro_header_html[] = '
												<div class="' . implode( ' ' , $wr_classes_item_outer ) . '"' . ( ( ! ( $wr_hb_settings['type'] == 'horizontal' && $wr_hb_item['layoutStyle'] == 'text' ) ) ? ' data-effect-vertical="' . esc_attr( $wr_hb_item['subMenu']['effectNormalVertical'] ) . '"' : NULL ) . '>';
								...
								case 'sidebar':
									...
									$wr_nitro_header_html[] = '
										<div class="' . implode( ' ' , $wr_classes_item ) . '" ' . ( $wr_hb_item['ID'] ? ( 'id="' . esc_attr( $wr_hb_item['ID'] ) . '"' ) : NULL ) . '>';
											...
											<div class="icon-sidebar">
												<div class="wr-burger-sidebar"><span></span></div>
											</div>';
									...
									// Set color for icon
									if( $wr_hb_item['iconColor'] ) {
										$wr_nitro_header_css[] = '.hb-s' . ( $key + 1 ) . 'i' . ( $key_item + 1 ) .' .wr-burger-sidebar{color:' . esc_attr( $wr_hb_item['iconColor'] ) . '}';
									}

									// Set hover color for icon
									if( $wr_hb_item['hoverIconColor'] ) {
										$wr_nitro_header_css[] = '.hb-s' . ( $key + 1 ) . 'i' . ( $key_item + 1 ) .' .wr-burger-sidebar:hover{color:' . esc_attr( $wr_hb_item['hoverIconColor'] ) . '}';
									}
								...
								case 'text':
									...
									$wr_nitro_header_html[] = '
										<div class="' . implode( ' ' , $wr_classes_item ) . '"' . ( $wr_hb_item['ID'] ? ( ' id="' . esc_attr( $wr_hb_item['ID'] ) . '"' ) : NULL ) . '>
											<div class="content-text">' . do_shortcode( wp_kses_post( $wr_hb_item['content'] ) ) . '</div>
										</div>';
										...
										if( $wr_hb_item['socialList'] ) {
											...
											foreach( $wr_hb_item['socialList'] as $key_sub_item => $val_sub_item ) {
												...
												$wr_nitro_header_html[] = ( isset( $wr_nitro_options[ $key_sub_item ] ) && $wr_nitro_options[ $key_sub_item ] != '' )
													...
													: 'wr-color-hover-' . $key_sub_item ) . '" href="' . esc_attr( $wr_nitro_options[ $key_sub_item ] ) . '" title="' . str_replace( '-', ' ', ucfirst( esc_attr( $key_sub_item ) ) ) . '" target="_blank"><i class="fa fa-' . $key_sub_item . '"></i></a>' ) : NULL;
											}
										}
									...
									$wr_shopping_cart = '<div class="' . implode( ' ' , $wr_classes_item ) . '" ' . ( $wr_hb_item['type'] == 'dropdown' ? ( 'data-margin-top="' . ( ( $wr_hb_item['marginTop'] !== '' || $wr_hb_settings['type'] == 'vertical' ) ? absint( $wr_hb_item['marginTop'] ) : 'empty' ) . '"' ) : NULL ) . ( $wr_hb_item['ID'] ? ( ' id="' . esc_attr( $wr_hb_item['ID'] ) . '"' ) : '' ) . '>';
										...
										$wr_shopping_cart .= '<span ' . ( $wr_hb_item['type'] == 'sidebar' ? ( ' ' . 'data-animation="' . esc_attr( $wr_hb_item['animationSidebar'] ) . '" data-position="' . esc_attr( $wr_hb_item['position'] ) . '"' ) : NULL ) . ' class="cart-control ' . ( $wr_hb_item['type'] == 'sidebar' ? 'cart-control-sidebar' : NULL ) . '">';
											...
											if ( $wr_hb_item['showCartInfo'] == 'item_number' || $wr_hb_item['showCartInfo'] == 'number_price' ) {
												$wr_shopping_cart .= '<span class="count">' . esc_html( $woocommerce->cart->get_cart_contents_count() ) . '</span>';
											}

										$wr_shopping_cart .= '</span>';

										if ( $wr_hb_item['showCartInfo'] == 'total_price' || $wr_hb_item['showCartInfo'] == 'number_price' ) {
											$wr_shopping_cart .= '<span class="mini-price ' . ( $wr_hb_item['showCartInfo'] == 'number_price' ? 'number-price' : '' ) . '">' . $woocommerce->cart->get_cart_total() . '</span>';
										}
										...
										if( $wr_hb_item['type'] == 'dropdown' ) {
											$wr_shopping_cart .= '<a class="link-cart" href="' . esc_url( $woocommerce->cart->get_cart_url() ) . '"></a>';
										}
								...
								case 'currency':
									...
									if( $data_currency ) {
											...
											if( isset( $current_active['id'] ) && $current_active['id'] == $currency['id'] ) {
												$item_active = '
													<div class="item active-currency">
														' . ( $wr_hb_item['show_flag'] ? '<img width="37" src="' . ( $currency['flag'] ? $currency['flag'] : WR_CC_URL . "assets/images/placeholder-flag.jpg" ) . '" />' : NULL ) . '
														<span class="name">' . $currency['name'] . '</span>' .
														( $currency['des'] ? '<span class="minus">&nbsp;-&nbsp;</span>
														<span class="des">' . $currency['des'] . '</span>' : NULL ) . '
														<span class="symbol">&nbsp;(' . WR_Currency_Hook::list_currency( $currency['symbol'] ) . ')</span>
										...
										if( $item_active ) {
											...
											$currency_content .= '
											<div class="item active-currency">
												<span class="des">' . __( 'Select currency' ,'wr-nitro' ) . '</span>
												<i class="fa fa-angle-down" aria-hidden="true"></i>
											</div>';
										}
			...
			if( $wr_hb_row['sticky'] == 1 ) {
				...
				if( $wr_hb_row['textColorSticky'] ) {
					...
					.header .sticky-row-scroll .section-' . ( $key + 1 ) .' .site-navigator > li:hover > .menu-item-link { color: inherit !important; }
					...
					.header .sticky-row-scroll .section-' . ( $key + 1 ) . ' .wr-burger-sidebar{color:' . $wr_color_sticky . '}
					';
...
if( (int) $wr_nitro_options['wr_layout_offset'] > 0 ) {
	...
	$wr_nitro_header_css[] = '
		@media only screen and (min-width: 1024px) {
			...
			body.admin-bar .hd-overlay-menu,
			body.admin-bar .hd-overlay-sidebar,
...
WR_Nitro_Header_Builder::prop( 'fonts', $wr_nitro_header_fonts );
