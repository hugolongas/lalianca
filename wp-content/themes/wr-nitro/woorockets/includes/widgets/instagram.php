...
class WR_Nitro_Widgets_Instagram extends WP_Widget {
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'id' ) ); ?>"><?php echo sprintf( __( 'Instagram user ID (<a href="%1$s" target="_blank">Lookup your User ID</a>)', 'wr-nitro' ), $lookup_url ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'token' ) ); ?>"><?php echo sprintf( __( 'Access token (<a href="%1$s" target="_blank">Generate access token</a>)', 'wr-nitro' ), $generate_token ); ?></label>
