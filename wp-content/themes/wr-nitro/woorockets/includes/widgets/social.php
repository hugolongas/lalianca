...
class WR_Nitro_Widgets_Social extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$wr_channels = array( 'facebook', 'twitter', 'instagram', 'linkedin', 'pinterest', 'dribbble', 'behance', 'flickr', 'google-plus', 'medium', 'skype',  'slack', 'tumblr', 'vimeo', 'yahoo', 'youtube', 'rss' );
		...
		foreach ( $wr_channels as $key => $value ) {
			if ( isset( $wr_nitro_options[ $value ] ) && $wr_nitro_options[ $value ] ) {
				...
				if ( in_array( $value_insert , $list_social ) ) {
					$output .='<a class="' . esc_attr( $value ) . ' dib pr tc" target="_blank" href="' . $wr_nitro_options[ $value ] . '"><i class="fa fa-' . esc_attr( $value ) . '"></i><span class="tooltip ab ts-03">' . str_replace( '-', ' ', esc_html( $value ) ) . '</span></a>';
				}
			}
		}
		$output .= '</div>';
	...
	?>
		...
		<div>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php printf( __( 'Select link to show (<a href="%s" target="_blank">Change your social links</a>)', 'wr-nitro' ), admin_url() . 'customize.php?autofocus[section]=social' ); ?></label>
		...
		<p>
			...
			<input class="widget-color-picker" type="text" id="<?php echo $this->get_field_id( 'icon_color' ); ?>" name="<?php echo $this->get_field_name( 'icon_color' ); ?>" value="<?php echo esc_attr( $icon_color ); ?>" /><br />
		</p>
		...
		<div class="wr-style-custom" <?php echo ( ( $style != 'style-custom' ) ? 'style="display: none"' : NULL ); ?>>
			<p>
				...
				<input class="widget-color-picker" type="text" id="<?php echo $this->get_field_id( 'background_color' ); ?>" name="<?php echo $this->get_field_name( 'background_color' ); ?>" value="<?php echo esc_attr( $background_color ); ?>" /><br />
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'border_color' ) ); ?>"><?php esc_html_e( 'Border color:', 'wr-nitro' ); ?></label><br />
				<input class="widget-color-picker" type="text" id="<?php echo $this->get_field_id( 'border_color' ); ?>" name="<?php echo $this->get_field_name( 'border_color' ); ?>" value="<?php echo esc_attr( $border_color ); ?>" /><br />
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'border_width' ) ); ?>"><?php esc_html_e( 'Border width:', 'wr-nitro' ); ?></label>
				<input class="tiny-text" type="number" id="<?php echo $this->get_field_id( 'border_width' ); ?>" name="<?php echo $this->get_field_name( 'border_width' ); ?>" value="<?php echo esc_attr( $border_width ); ?>" />
			</p>
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'border_radius' ) ); ?>"><?php esc_html_e( 'Border radius:', 'wr-nitro' ); ?></label>
				<input class="tiny-text" type="number" id="<?php echo $this->get_field_id( 'border_radius' ); ?>" name="<?php echo $this->get_field_name( 'border_radius' ); ?>" value="<?php echo esc_attr( $border_radius ); ?>" />
			</p>
		</div>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'icon_spacing' ) ); ?>"><?php esc_html_e( 'Icon spacing:', 'wr-nitro' ); ?></label>
			<input class="tiny-text" type="number" id="<?php echo $this->get_field_id( 'icon_spacing' ); ?>" name="<?php echo $this->get_field_name( 'icon_spacing' ); ?>" value="<?php echo esc_attr( $icon_spacing ); ?>" />
		</p>
		<p>
