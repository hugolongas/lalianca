...
class WR_Nitro_Meta_Box extends RW_Meta_Box {
	...
	public static function meta_boxes( $meta_boxes ) {
		...
		$product_style = $wr_nitro_options['wc_single_style'];
		...
		$meta_boxes[] = array(
			...
			'fields'     => array(
				...
				array(
					...
					'type'   => 'toggle',
					'std'    => 0,
					'hidden' => array( 'page_layout_bg_image', '=', '' ),
					'tab'    => esc_html__( 'Page Content', 'wr-nitro' ),
				),
				array(
					'name'   => esc_html__( 'Mask Overlay Color', 'wr-nitro' ),
		...
		if ( $product_style == '1' || $product_style == '4' ) {
			$meta_boxes[] = array(
				'id'         => 'image_bg',
				'post_types' => 'product',
				'title'      => esc_html__( 'Preview Image Settings', 'wr-nitro' ),
				'context'    => 'side',
				'priority'   => 'low',
				'fields'     => array(
					array(
		...
		$meta_boxes[] = array(
			'id'         => 'product_builder',
			'post_types' => 'product',
			'title'      => esc_html__( 'Enable Product Builder', 'wr-nitro' ),
			'context'    => 'normal',
			'priority'   => 'high',
			'fields'     => array(
				array(
	...
	public static function outside_conditions() {
		...
		if ( isset( $post->post_type ) && in_array( $post->post_type, $types ) ) {
			$conditions['.wr-tab-pagetitle, .wr-tab-pagecontent'] = array(
				'hidden' => array( 'global_opt', 1 )
			);
			return $conditions;
		}
