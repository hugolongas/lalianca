...
class WR_Nitro_Helper {
	...
	public static function get_author() {
		return '<span class="entry-author"><i class="fa fa-user"></i><a href="' . get_author_posts_url( get_the_author_meta( 'ID' ) ) . '">' . get_the_author() . '</a></span>';
	}
	...
	public static function get_posted_on() {
		$posted_on = sprintf(
			'<time class="published updated" datetime="%1$s"><a href="%2$s" rel="bookmark">%3$s</a></time>',
						...
						<div class="comment-content-wrap overlay_bg">
							...
							<header class="comment-meta">
								...
								<a href="<?php echo '' . get_comment_link( $comment->comment_ID ); ?>">
									<time <?php self::schema_metadata( array( 'context' => 'comment_time' ) ); ?>>
										<span> - </span>
										<?php sprintf( __( '%1$s', 'wr-nitro' ), comment_date() ); ?>
									</time>
								</a>
							</header>
							<section class="comment-content comment" <?php self::schema_metadata( array( 'context' => 'comment_text' ) ); ?>>
								<?php comment_text(); ?>
							</section>
	...
	public static function remove_action( $action, $class, $priority ) {
		...
		if ( ! empty( $wp_filter[ $action ] [ $priority ] ) ) {
			foreach( $wp_filter[ $action ] [ $priority ] as $key => $val ) {
				if( ! empty( $val['function'][0] ) && ! empty( $val['function'][1] ) && get_class( $val['function'][0] ) == $class[0] && $val['function'][1] == $class[1] ) {
					unset( $wp_filter[ $action ] [ $priority ][ $key ] );
				}
			}
		}
	}
