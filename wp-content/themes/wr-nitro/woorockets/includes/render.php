...
class WR_Nitro_Render {
	...
	public static function enqueue_scripts() {
		...
		if ( intval( $wr_nitro_options['wc_single_image_zoom'] ) && ! wp_is_mobile() && function_exists( 'is_product' ) && is_product() ) {
			wp_enqueue_script( 'wr-nitro-elevateZoom', get_template_directory_uri() . '/assets/3rd-party/elevatezoom/jquery.elevateZoom.min.js' , array(), false, true );
		}
	...
	public static function fonts_url() {
		...
		// Header fonts
		$header_fonts = WR_Nitro_Header_Builder::prop( 'fonts' );

		if ( count( $header_fonts ) ) {
			foreach ( $header_fonts as $font_name => $font_weight ) {
				$font_families[ $font_name ] = isset( $font_families[ $font_name ] ) ? array_unique( array_merge( $font_families[ $font_name ], $font_weight ) ) : $font_weight;
			}
		}
	...
	public static function custom_styles() {
		...
		$product_single_style  = $wr_nitro_options['wc_single_style'];
		...
		$css[] = 'body {';
			...
			h5 {
				...
				line-height: ' . esc_attr( $heading_font_size ) . 'px;
			}
			h6 {
		...
		if ( '1' == $product_single_style || '4' == $product_single_style ) {

			if ( $image_bg_color == '' ) {
				$css[] = '.product .p-single-top { background: ' . esc_attr( $product_single_bg ) . '; }';

			} else {
				$css[] = '.product .p-single-top { background: ' . esc_attr( $image_bg_color ) . '; }';
			}
		}

		if ( '2' == $product_single_style && ! empty( $offset ) ) {
			$css[] = '
		...
		switch ( $wr_nitro_options['wr_page_layout'] ) {
			...
			case 'left-sidebar' :
					$css[] = '
						...
						.page-content .primary-sidebar {
							width: ' . esc_attr( $page_sidebar_width ) . 'px;
							right: calc(100% - ' . esc_attr( $page_sidebar_width ) . 'px);
						}
						.page-content .main-content {
							left: ' . esc_attr( $page_sidebar_width ) . 'px;
							width: calc(100% - ' . esc_attr( $page_sidebar_width ) . 'px);
						}
					';
				break;
		...
		switch ( $blog_layout ) {
			...
			case 'left-sidebar' :
					$css[] = '
						...
						.blog .primary-sidebar {
							width: ' . esc_attr( $blog_sidebar_width ) . 'px;
							right: calc(100% - ' . esc_attr( $blog_sidebar_width ) . 'px);
						}
						.blog .main-content {
							left: ' . esc_attr( $blog_sidebar_width ) . 'px;
							width: calc(100% - ' . esc_attr( $blog_sidebar_width ) . 'px);
						}
					';
				break;
		...
		switch ( $blog_single_layout ) {
			...
			case 'left-sidebar' :
					$css[] = '
						...
						.single-post .primary-sidebar {
							width: ' . esc_attr( $blog_single_sidebar_width ) . 'px;
							right: calc(100% - ' . esc_attr( $blog_single_sidebar_width ) . 'px);
						}
						.single-attachment .main-content,
						.single-post .main-content {
							left: ' . esc_attr( $blog_single_sidebar_width ) . 'px;
							width: calc(100% - ' . esc_attr( $blog_single_sidebar_width ) . 'px);
						}
					';
				break;
		...
		switch ( $wc_single_layout ) {
			...
			case 'left-sidebar' :
					$css[] = '
						...
						.style-2 #shop-sidebar {
							width: ' . absint( $wc_single_sidebar_width ) . 'px;
							right: calc(100% - ' . absint( $wc_single_sidebar_width ) . 'px);
						}
						.style-2 #shop-detail {
							left: ' . absint( $wc_single_sidebar_width ) . 'px;
							width: calc(100% - ' . absint( $wc_single_sidebar_width ) . 'px);
						}
					';
				break;
		...
		if ( ! empty( $line_color ) ) {
			$css[] = '
				...
				.quickview-modal .quickview-button > div a,
		...
		if ( ! empty( $overlay_bg ) ) {
			...
			$css[] = '
				@media (max-width: 768px) {
					.woocommerce-page table.shop_table_responsive tr:nth-child(odd) {
						background-color: ' . esc_attr( $overlay_bg ) . ';
					}
				}
			';
		...
		if ( ! empty( $content_bg_color ) ) {
			$css[] = '
				...
				.wr-pricing-table.style-1 .pricing-item .inner,
				.select2-results,
				.woocommerce-account .wrapper-outer,
				.woocommerce-checkout .wrapper-outer,
				.woocommerce-cart .wrapper-outer,
	...
	.hb-cart-outer .dark-style .widget_shopping_cart_content .mini_cart_item .info-item .action-item a:hover,
	.hb-minicart .mini_cart_item .info-item .title-item a:hover,
	.widget_shopping_cart_content .info-item .action-item .edit-cart:hover,
	.widget_shopping_cart_content .product_list_widget .action-item .remove:hover,
	.hb-cart-outer .dark-style .mini_cart_item .info-item .action-item .edit-cart:hover,
...
CSS;
	...
	public static function body_class( $classes ) {
		...
		if ( $wr_nitro_options['under_construction'] && ( is_customize_preview() || ! is_super_admin() ) ) {
			$classes[] = 'maintenance';
