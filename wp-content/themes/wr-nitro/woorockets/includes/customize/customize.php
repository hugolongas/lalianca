...
class WR_Nitro_Customize {
	 ...
	 * @return  array
	 */
	public static function sanitize_canvas_sidebar_widgets( $value ) {
		// Make sure value is an array.
		if ( ! is_array( $value ) ) {
			$value = array( $value );
		}

		// Loop thru array of value and sanitize each.
		$widgets = array();

		foreach ( $value as $k => $v ) {
			$widgets[] = sanitize_key( $v );
		}

		// Save canvas sidebar widgets.
		...
		$sidebars_widgets['canvas-sidebar'] = $widgets;
	...
	public static function get_options() {
		...
		if ( ! isset( $prepared ) ) {
			...
			if ( is_customize_preview() ) {
				if ( isset( $_POST['customized'] ) && '{}' != $_POST['customized'] && '' != $_POST['customized'] ) {
					self::$theme_options = array_merge(
						self::$theme_options,
						json_decode( stripslashes( $_POST['customized'] ), true )
					);
				}
			}
