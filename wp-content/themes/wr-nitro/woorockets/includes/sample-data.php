...
class WR_Nitro_Sample_Data {
	...
	public static function initialize() {
		...
		if ( 'remote' == self::$demo_images_storage && false !== get_transient( 'wr_nitro_sample_package' ) ) {
	...
	public static function get_sample_packages( $package = null ) {
		...
		if ( ! $sample_packages ) {
			...
			if ( 'sample-data.json' == self::$server ) {
				if ( ! @is_file( $sample_packages = implode( '/', array_slice( explode( '/', str_replace( '\\', '/', __FILE__ ) ), 0, -1 ) ) . '/sample-data.json' ) ) {
					if ( $return ) {
						return false;
					}

					wp_send_json_error( __( 'Failed to get available sample data packages.', 'wr-nitro' ) );
				}

				// Read sample package definition.
				global $wp_filesystem;

				$sample_packages = $wp_filesystem->get_contents( $sample_packages );
			}
				...
				if ( is_wp_error( $sample_packages ) ) {
					...
					wp_send_json_error( __( 'Failed to retrieve available sample data packages.', 'wr-nitro' ) );
				}
						...
						<ul>
							...
							<li>
								...
								<ol>
									...
									<li><?php echo esc_html( str_replace( $_SERVER['DOCUMENT_ROOT'], '', $dir ) ); ?></li>
									<?php endforeach; ?>
								</ol>
							</li>
							<?php else : ?>
							<li><?php printf( __( 'Installing sample data will replace the content of current website with <a href="%1$s" target="_blank"><strong>%2$s</strong> demo</a>.', 'wr-nitro' ), $package['demo'], $package['name'] ); ?></li>
				...
				<div id="sample-data-installation-step-2" class="hidden" data-package="<?php echo esc_attr( $package['id'] ); ?>">
					...
					<div id="wr-install-sample-data-manually" class="hidden">
						<form enctype="multipart/form-data" method="post" target="wr-upload-sample-data" action="<?php
							echo '' . admin_url( 'admin-ajax.php?action=nitro_install_sample_data&step=2&package=' . $package['id'] );
						?>">
							<ol>
								<li>
									<?php esc_html_e( 'Please download sample data package manually', 'wr-nitro' ); ?>
									<a href="<?php echo esc_url( $package['download'] ); ?>" class="button" target="_blank">
	...
	protected static function download_sample_package( $package ) {
		...
		if ( 'POST' == $_SERVER['REQUEST_METHOD'] && isset( $_FILES['package'] ) ) {
	...
	protected static function import_sample_data( $package ) {
		...
		if ( ! @is_dir( $path ) ) {
			wp_send_json_error( __( 'Failed to extract downloaded package to file system.', 'wr-nitro' ) );
		}
		...
		if ( ! count( $xml ) ) {
			wp_send_json_error( __( 'Invalid sample data package.', 'wr-nitro' ) );
		}
		...
		if ( ! $xml ) {
			wp_send_json_error( __( 'Unable to parse sample data declaration file.', 'wr-nitro' ) );
		}
		...
		$wp_filesystem->delete( $path . '.zip' );

		foreach ( glob( "{$path}/*.xml" ) as $xml ) {
			$wp_filesystem->delete( $xml );
		}
	...
	protected static function import_full_demo_site( $package, $xml, $theme, $child = null ) {
		...
		if ( $folders && count( $folders ) ) {
			...
			foreach ( $folders as $folder ) {
				...
				if ( $wp_filesystem->exists( $src ) && $wp_filesystem->is_dir( $src ) ) {
					$des = ABSPATH . $folder;

					if ( ! $wp_filesystem->exists( $des ) ) {
						// Store demo folder to remove later when uninstall sample data.
						$demo_folders[] = ( string ) $folder;
					}

					$wp_filesystem->move( $src, $des, true );
				}
			}
		}
		...
		if ( ! self::prepare_directory( $backup_dir ) ) {
			wp_send_json_error( __( 'Failed to create directory to store database backup file.', 'wr-nitro' ) );
		}
						...
						if ( $results ) {
							foreach ( $results as $result ) {
								...
								if ( strlen( $queries . $query ) > self::$max_backup_file_size ) {
									...
									$wp_filesystem->put_contents( $path, $queries );
		...
		delete_transient( 'wr_nitro_sample_package' );
		...
		set_transient( 'wr_nitro_sample_package', $package['id'] );
	...
	protected static function restore_backup_data( $package ) {
		...
		if ( count( $backup ) ) {
			...
			foreach ( ( array ) $backup as $file ) {
				...
				foreach ( explode( ";\n", $wp_filesystem->get_contents( $file ) ) as $query ) {
					$wpdb->query( "{$query};" );
				}
			...
			if ( $demo_assets ) {
				...
				foreach ( $demo_assets as $url => $sizes ) {
					...
					if ( $wp_filesystem->exists( $path ) && $wp_filesystem->is_file( $path ) ) {
			...
			if ( $demo_folders ) {
				foreach ( $demo_folders as $folder ) {
					...
					if ( $wp_filesystem->exists( $folder ) && $wp_filesystem->is_dir( $folder ) ) {
			...
			// Remove backup file.
			if ( is_array( $backup ) ) {
				$wp_filesystem->delete( dirname( current( $backup ) ), true );
			} else {
				$wp_filesystem->delete( $backup );
			}

			// Return previously installed sample data package.
			wp_cache_delete( 'wr_nitro_sample_package', 'transient' );
			wp_cache_delete( '_transient_wr_nitro_sample_package', 'options' );
			...
			$response[ 'last' ] = get_transient( 'wr_nitro_sample_package' );

			wp_send_json_success( $response );
		}

		wp_send_json_error( __( 'Not found any backup to restore.', 'wr-nitro' ) );
	}
	...
	protected static function download( $uri, $target = '' ) {
		...
		if ( ! empty( $target ) ) {
			...
			// Move file.
			if ( ! $wp_filesystem->move( $result, $target, true ) ) {
				return false;
			}

			$content = $wp_filesystem->size( $target );
		} else {
