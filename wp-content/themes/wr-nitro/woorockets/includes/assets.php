...
class WR_Nitro_Assets {
	...
	public static function scripts( $level_1, $level_2 = NULL, $wp_content = true ) {
		...
		$data = array(
			...
			'page' => array(
				...
				'rwmb-media'                        => $wp_content . '/themes/wr-nitro/assets/woorockets/js/admin/override/plugins/meta-box/media.js',
				...
				'conditional-logic'                 => $wp_content . '/plugins/meta-box-conditional-logic/assets/js/conditional-logic.js',
	...
	public static function optimize_site_assets() {
		...
		$is_shop    = ( ( function_exists( 'is_shop' ) && is_shop() ) || is_post_type_archive( 'product' ) || ( function_exists( 'is_product_category' ) && is_product_category() ) || ( function_exists( 'is_product_tag' ) && is_product_tag() ) );
		...
		if ( $is_product || is_post_type_archive( 'nitro-gallery' ) || is_singular( 'nitro-gallery' ) || ( is_single() && 'gallery' == $format ) || is_tax( 'gallery_cat' ) ) {
		...
		if ( isset( $post ) && has_shortcode( $post->post_content, 'yith_woocompare_table' ) ) {
			wp_enqueue_style( 'wr-nitro-woocommerce' );
		}
		...
		if ( ! is_404() ) {
			...
			if ( $is_shop || $is_product || $is_wc || is_search() || ( isset( $post ) && ( has_shortcode( $post->post_content, 'nitro_buy_now' ) || has_shortcode( $post->post_content, 'woocommerce_order_tracking' ) || has_shortcode( $post->post_content, 'nitro_product_attribute' ) || has_shortcode( $post->post_content, 'nitro_product' ) || has_shortcode( $post->post_content, 'nitro_products' ) || has_shortcode( $post->post_content, 'add_to_cart' ) ) ) ) {
				wp_enqueue_style( 'wr-nitro-woocommerce' );
				...
				foreach ( $sources as $source ) {
					...
					if ( strlen( $compressed ) >= $max_size ) {
						if ( $num_files == 1 ) {
							...
							if ( ! $wp_filesystem->put_contents( substr( $compression, 0, -4 ) . "-{$num_files}.css", $compressed ) ) {
								return;
							}
						}
	...
	public static function minify_css( $stylesheet ) {
		...
		$css = ( @is_file( $stylesheet ) ) ? $wp_filesystem->get_contents( $stylesheet ) : $stylesheet;
