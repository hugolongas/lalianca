		...
		<div class="product__action-inner icon_color">
			<?php
				...
				if ( $wr_nitro_options['wc_buynow_btn'] && $product->product_type == 'simple' && ! $wr_catalog_mode && $product->is_in_stock() && ! WR_Nitro_Helper::check_gravityforms( $post->ID ) ) {
					echo '<a class="product__btn color-dark btn-buynow hover-primary pr" href="#" data-product-id="' . get_the_ID() . '"><i class="nitro-icon-' . esc_attr( $wr_icons ) . '-quickbuy"></i><span class="tooltip ab">' . esc_html__( 'Buy Now', 'wr-nitro' ) . '</span></a>';
				}
