...
<div class="product__wrap product-btn-center tc">
	<div class="product__image oh pr <?php echo esc_attr( $wr_hover_class ); ?>">
		...
		<div class="product__action icon_color pa fc">
			<?php
				// Product quickview
				echo '<a class="product__btn bts-40 btn-quickview color-dark bgw dib pr hover-primary" href="#0" data-prod="' . esc_attr( $post->ID ) . '"><i class="nitro-icon-' . esc_attr( $wr_icons ) . '-quickview"></i><span class="tooltip ab">' . esc_html__( 'Quick View', 'wr-nitro' ) . '</span></a>';
				// Quick buy button
				if ( $wr_nitro_options['wc_buynow_btn'] && $product->product_type == 'simple' && ! $wr_catalog_mode && $product->is_in_stock() && ! WR_Nitro_Helper::check_gravityforms( $post->ID ) ) {
					echo '<a class="product__btn bts-40 color-dark bgw db dib pr btn-buynow hover-primary" href="#" data-product-id="' . get_the_ID() . '"><i class="nitro-icon-' . esc_attr( $wr_icons ) . '-quickbuy"></i><span class="tooltip ab">' . esc_html__( 'Buy Now', 'wr-nitro' ) . '</span></a>';
				}
				...
				if ( class_exists( 'YITH_WOOCOMPARE' ) && $wr_show_compare && ! $wr_catalog_mode ) {
					echo '
						<div class="product__compare">
							<a class="product__btn bts-40 bgw mg0 dib" href="#"><i class="nitro-icon-' . esc_attr( $wr_icons ) . '-compare"></i><span class="tooltip ab">' . esc_html__( 'Compare', 'wr-nitro' ) . '</span></a>
	...
	<?php
		...
		if ( ! $wr_catalog_mode || $wr_show_price ) {
			echo '<div class="product__price mgb10">';
