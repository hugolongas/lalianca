...
<div itemscope itemtype="<?php echo woocommerce_get_product_schema(); ?>" id="product-<?php the_ID(); ?>" <?php post_class( 'style-5 social-circle ' . $layout . ( is_customize_preview() ? ' customizable customize-section-product_single' : '' ) ); ?>>
				...
				<div class="row">
					<div class="w667-12 cm-8">
						<div class="block-info overlay_bg pd30 btb nitro-line">
							<div class="product-categories tu"><?php echo $product->get_categories(); ?></div>
							<h1 class="product-title mg0 mgb20" itemprop="name"><?php the_title(); ?></h1>
									...
									if ( $wr_catalog_mode && $wr_show_button ) {
										echo '<div class="p-single-action">';
											if ( $wr_show_button_action == 'simple' ) {
												echo '<a target="_blank" class="button wr-btn wr-btn-solid wr-btn-custom" href="' . esc_attr( $wr_show_button_text ) . '">' . esc_attr( $wr_nitro_options['wc_archive_catalog_mode_button_text'] ) . '</a>';
											} else {
								...
								<div class="product_meta">
									...
									<?php if ( wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type( 'variable' ) ) ) : ?>
										...
										<div class="sku_wrapper fc jcsb nitro-line">
											<span class="fwb tu"><?php esc_html_e( 'SKU', 'wr-nitro' ); ?> </span> <span class="sku" itemprop="sku"><?php echo ( $sku = $product->get_sku() ) ? $sku : esc_html__( 'N/A', 'wr-nitro' ); ?></span>
										</div>
