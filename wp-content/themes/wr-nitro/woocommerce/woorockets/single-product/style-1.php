...
<div itemscope itemtype="<?php echo woocommerce_get_product_schema(); ?>" id="product-<?php the_ID(); ?>" <?php post_class( 'style-1' . ( is_customize_preview() ? ' customizable customize-section-product_single' : '' ) . $class_gravity_form ); ?>>
			...
			if ( $wr_catalog_mode && $wr_show_button ) {
				echo '<div class="p-single-action fr clearfix">';
					if ( $wr_show_button_action == 'simple' ) {
						echo '<a target="_blank" class="button wr-btn wr-btn-solid" href="' . esc_attr( $wr_show_button_text ) . '">' . esc_attr( $wr_nitro_options['wc_archive_catalog_mode_button_text'] ) . '</a>';
					} else {
