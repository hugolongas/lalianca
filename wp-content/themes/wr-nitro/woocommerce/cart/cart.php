...
<form class="woocommerce-cart pd30" action="<?php echo esc_url( WC()->cart->get_cart_url() ); ?>" method="post">
				...
				<tbody>
					...
					foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
						...
						if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
							?>
							<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ) . ' product-type-' . $_product->product_type; ?>">
