...
<?php if ( $has_downloads ) : ?>
	...
	<h1><?php esc_html_e( 'Manage Downloads', 'wr-nitro' ); ?></h1>
							...
							<?php
								$actions = array(
									'download'  => array(
										...
										'name' => __( 'Download', 'wr-nitro' )
									)
								);

								if ( $actions = apply_filters( 'woocommerce_account_download_actions', $actions, $download ) ) {
									foreach ( $actions as $key => $action ) {
										echo '<a href="' . esc_url( $action['url'] ) . '" class="wr-btn wr-btn-outline woocommerce-Button ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a>';
									}
								}
							?>
...
<?php else : ?>
	<div class="woocommerce-Message woocommerce-Message--info woocommerce-info">
		<a class="woocommerce-Button button" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
			<?php esc_html_e( 'Go Shop', 'wr-nitro' ) ?>
		</a>
		<?php esc_html_e( 'No downloads available yet.', 'wr-nitro' ); ?>
	</div>
<?php endif; ?>
