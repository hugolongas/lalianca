;(function($)
{
	...
	function getFieldValue(fieldName, $scope)
	{
		...
		if ( null !== selector )
		{
			...
			// Get value for field of type Image Advanced.
			if ( $(selector).hasClass( 'rwmb-image_advanced' ) ) {
				var val = $(selector).siblings( '.rwmb-media-view' ).find( selector ).val();
	...
	if (window.eventSource.length > 1) {
		var eventSource = window.eventSource.split(',');

		for (var i = 0; i < eventSource.length; i++) {
			var source = $(eventSource[i]);

			if (source.hasClass('rwmb-image_advanced')) {
				$(document).on('selection-change', function() {
					runConditionalLogic(conditions);
				});
			} else {
				source.live('change keyup click', function() {
					runConditionalLogic(conditions);
				});
			}
		}
