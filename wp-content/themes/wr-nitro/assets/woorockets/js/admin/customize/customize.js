					...
					if ( event.target.value != '' && event.target.value.length > 2 ) {
						...
						$( 'li.customize-control .customize-control-title' ).each( function( i, e ) {
							var item = $( e ).closest( 'li.customize-control' ),
								panel = $( e ).closest( 'li.control-section' );

							if ( ! item.hasClass( 'hidden' ) && panel.css( 'display' ) != 'none' ) {
								if ( $( e ).text().toLowerCase().indexOf( event.target.value.toLowerCase() ) > -1 ) {
									// Build path to option.
									var path = [], parent = e.parentNode, title;

									while ( parent.id != 'customize-theme-controls' && parent.nodeName != 'BODY' ) {
										if ( parent.classList.contains( 'control-section' ) ) {
									...
									result = $( '<a href="javascript:void(0)">' )
										...
										.click( function( event ) {
											...
											var panel = target.closest( 'li.control-section' );
