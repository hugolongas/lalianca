...
(function($) {
	...
	api( 'btn_line_height', function( value ) {
		value.bind( function( to ) {
			...
			$( '.quantity .qty a' ).css({ 'height' : ( to / 2 ) + 'px', 'height' : ( to / 2 )+ 'px' });
		} );
	} );
	...
	// Under construction
	api( 'under_construction_style', function( value ) {
		value.bind( function( to ) {
			$( 'body' ).removeClass( function() {
				var toReturn = '',
					classes = this.className.split(' ');
				for(var i = 0; i < classes.length; i++ ) {
					if( /maintenance-style-\d{1,3}/.test( classes[i] ) ) {
						toReturn += classes[i] +' ';
					}
				}
				return toReturn;
			}).addClass( 'maintenance-style-' + to );
		} );
	} );
	api( 'under_construction_bg_color', function( value ) {
		value.bind( function( to ) {
			$( '.maintenance .wrapper-outer' ).css( 'background-color', to )
		} );
	} );
	api( 'under_construction_bg_image_size', function( value ) {
		value.bind( function( to ) {
			$( '.maintenance .wrapper-outer' ).css( 'background-size', to );
		} );
	} );
	api( 'under_construction_bg_image_repeat', function( value ) {
		value.bind( function( to ) {
			$( '.maintenance .wrapper-outer' ).css( 'background-repeat', to );
		} );
	} );
	api( 'under_construction_bg_image_position', function( value ) {
		value.bind( function( to ) {
			$( '.maintenance .wrapper-outer' ).css( 'background-position', to );
		} );
	} );
	api( 'under_construction_bg_image_attachment', function( value ) {
		value.bind( function( to ) {
			$( '.maintenance .wrapper-outer' ).css( 'background-attachment', to );
		} );
	} );
	api( 'under_construction_title', function( value ) {
		value.bind( function( to ) {
			$( '.maintenance .maintenance-title' ).html( to );
		} );
	} );
	api( 'under_construction_message', function( value ) {
		value.bind( function( to ) {
			$( '.maintenance .maintenance-message' ).html( to );
		} );
	} );
