...
@section('content')
    ...
    <div class="options-menu">
        <a type="button" class="btn btn-outline-dark" href="{{ route('admin.categories.create') }}">Crear Categoria</a>
    </div>
...
@push('scripts')
    <script>
        $(function() {
            ...
            $('#categories-table tbody').on('click', 'button', function(ev) {
                var data = datatable.row($(this).parents('tr')).data();
                var accion = $(this).attr("accion");
                switch (accion) {
                    case "eliminar": {
                        if (confirm("Segur que vols eliminar la categoria")) {
                            $.ajaxSetup({
                                headers: {
                            ...
                            $.ajax({
                                ...
                                success: function() {
                                    ...
                                    var alert =
                                        "<div id='custom-alert' class='alert alert-danger'>Categoria eliminada</div>";
